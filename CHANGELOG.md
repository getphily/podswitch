...
- **Native OBS Settings Layout:** Completely redesigned the Settings UI to match the native OBS settings dialog layout. Replaced the top tab bar with a left-aligned navigation list (`Podcast Format` and `General`) and a stacked page interface on the right.
- **Manual Switch Override:** The Auto Switcher will now automatically disable itself if a user manually clicks to change scenes in the OBS interface, ensuring it doesn't fight the user for control.
